...
const MAPPING = { Company: "accountName", Website: "accountRawDomain", "Max Leads": "maxLeadsPerAccountOverride" };
...
describe("target account list import", () => {
  ...
  it("stores the raw name/domain and the normalized domain", async () => {
    ...
    const acmeEntry = entries.find((e) => e.accountName === "Acme Inc");
    expect(acmeEntry?.accountRawDomain).toBe("https://www.acme.com");
    expect(acmeEntry?.accountNormalizedDomain).toBe("acme.com");
  });
  ...
  it("stores the per-entry cap override", async () => {
    ...
    const entries = await db.listEntry.findMany({ where: { listId: result.listId } });
    expect(entries.find((e) => e.accountName === "Acme Inc")?.maxLeadsPerAccountOverride).toBe(3);
    expect(entries.find((e) => e.accountName === "Globex")?.maxLeadsPerAccountOverride).toBeNull();
  });
  ...
  it("reports a per-row error for a row with neither name nor domain", async () => {
    ...
    const result = await importTargetAccountList(db, ops, {
      ...
      mapping: { Company: "accountName", Website: "accountRawDomain" },
    });
...
describe("resolveAccountCap (PRD decisions 3 and 4)", () => {
  ...
  it("prefers the entry override over the campaign default", async () => {
    ...
    expect(await resolveAccountCap(db, channel.id, "acme.com")).toBe(3);
  });
  ...
  it("falls back to the channel default when there is no override", async () => {
    ...
    expect(await resolveAccountCap(db, channel.id, "globex.com")).toBe(5);
  });

  it("returns null when neither an override nor a channel default is set", async () => {
    const { db, manager, client } = await setup();
    const campaign = await createCampaign(db, manager, {
    ...
    expect(await resolveAccountCap(db, channel.id, "acme.com")).toBeNull();
  });
});
...
describe("channel scoping (regression guard)", () => {
  ...
  it("does not leak a list attached to one channel onto a sibling channel", async () => {
    ...
    expect(await resolveAccountCap(db, channelA.id, "acme.com")).toBe(3);
    expect(await resolveAccountCap(db, channelB.id, "acme.com")).toBeNull();
  });
